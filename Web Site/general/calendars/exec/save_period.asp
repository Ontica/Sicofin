<%
  ...
  If (CLng(Session("uid")) = 0) Then
	...
	gsReturnPage = "../pages/period_editor.asp?calendar_id=" & Request.Form("txtCalendarId")
  ...
  Sub SaveItem(nItemId)
		...
		Set oRecordset = oCalendar.GetPeriodRS(Session("sAppServer"), CLng(nItemId))
		oRecordset("calendar_id") = CLng(Request.Form("txtCalendarId"))
		oRecordset("period_name")	= Request.Form("txtPeriodName")		
		oRecordset("from_date")		= CDate(Request.Form("txtFromDate"))		
		oRecordset("to_date")			= CDate(Request.Form("txtToDate"))		
		oRecordset("is_open")			= CLng(Request.Form("cboIsOpen"))		
		oCalendar.SavePeriod Session("sAppServer"), (oRecordset), CLng(nItemId)		
		oRecordset.Close
...
<tr>
	<td bgColor="khaki"><b>La información del período fue enviada satisfactoriamente.</b></td>
</tr>
<tr><td><br><b>¿Qué desea hacer?</b></td></tr>
<tr>
	<% If CLng(Request.Form("txtItemId")) = 0 Then %>
	<td>
		<a href='<%=gsReturnPage%>'>Agregar otro período</a>
	</td>	
	<% Else %>
	<td>
