<%
  ...
  If (CLng(Session("uid")) = 0) Then
	...
	Sub Main()
		...
		Set oRecordset   = oWorkgroup.Attributes(Application("sAppServer"), CLng(gnWorkgroupId))
		gsTitle					 = Left(oRecordset("participantName"), 40)
		Set oRecordset   = Nothing
		gsWorkgroupTasks = oWorkgroup.ParticipantTasksTable(Application("sAppServer"), CLng(gnWorkgroupId))
...
<FORM name=frmSend action='./exec/save_workgroup_tasks.asp' method=post>
		...
		<TD colspan=4 nowrap>
			<TABLE class='fullScrollMenu'>
				<TR class="fullScrollMenuHeader">					
					<TD nowrap>
						<b>Tareas asignadas al grupo de trabajo</b>
					</TD>
