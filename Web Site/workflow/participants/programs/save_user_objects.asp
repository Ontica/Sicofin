<%
  ...
  Sub Main()
		...
		gsParticipantName = oParticipant.Attributes(Session("sAppServer"), CLng(gnUserId)).Fields("participantName")
		If gnUserObjects > 0 Then			
			sArray = Request.Form("chkItems")(1)
			For i = 2 To gnUserObjects
				sArray = sArray & "," & Request.Form("chkItems")(i)
			Next
			oParticipant.ObjectPermissions Session("sAppServer"), CLng(gnEntityId), CLng(gnUserId), Split(sArray, ",")
		Else
			oParticipant.ObjectPermissions Session("sAppServer"), CLng(gnEntityId), CLng(gnUserId), Null
		End If
			...
			<TABLE class=fullScrollMenu>
				...
				<TR>
					<TD width=100% colspan=4>
						...
						<img src='../../images/separator.gif' width=100% height=1><br><br>
