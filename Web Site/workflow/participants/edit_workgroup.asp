...
function reactivate() {
	...
	if (confirm(sMsg)) {
		window.location.href = "./programs/change_status.asp?id=<%=gnWorkgroupId%>&status=1";
	}
}
...
function suspend() {
	...
	if (confirm(sMsg)) {
		window.location.href = "./programs/change_status.asp?id=<%=gnWorkgroupId%>&status=2";
	}
}
...
function delete_() {
	...
	if (confirm(sMsg)) {
		window.location.href = "./programs/change_status.asp?id=<%=gnWorkgroupId%>&status=3";
	}
}
...
<FORM name=frmSend action='./programs/save_workgroup.asp' method=post>
