...
function collapseAll() {
	...
	for (i = 0; i < oTableSections.length; i++) {
		...
		oTableSections[i].all.tags('TR')[0].all.tags('IMG')[0].src = '/empiria/images/expanded.gif';
...
function expandAll() {
	...
	for (i = 0; i < oTableSections.length; i++) {
		oTableSections[i].style.display = 'inline';
		oTableSections[i].value = Math.abs(oTableSections[i].value);
		oTableSections[i].all.tags('TR')[0].all.tags('IMG')[0].src = '/empiria/images/collapsed.gif';
	}	
	return false;
}
...
function showHelp(sItem) {	
	...
	var sURL = '/empiria/help_learning/help/online_help.asp?item=';
