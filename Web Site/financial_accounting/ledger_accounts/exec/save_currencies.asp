<%
  ...
  If (CLng(Session("uid")) = 0) Then
	...
	If Len(Request.QueryString("id")) <> 0 Then
		Call SaveCurrencies(CLng(Request.QueryString("id")))
  ...
  Sub SaveCurrencies(nStdAccountId)
		Dim aCurrencies(), oStdAccount, sCurrenciesList, i
		'******************************
		
		Set oStdAccount = Server.CreateObject("EFAStdActBS.CStdAccount")
		
		Redim aCurrencies(CLng(Request.Form("chkCurrencies").Count))
		For i = 1 To Request.Form("chkCurrencies").Count
			aCurrencies(i) = Request.Form("chkCurrencies")(i)
		Next
		sCurrenciesList = Join(aCurrencies, ",")
		oStdAccount.AddCurrencies Session("sAppServer"), CLng(nStdAccountId), _
														  CStr(sCurrenciesList), CDate(Request.Form("txtFromDate"))
