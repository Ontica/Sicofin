<%
  ...
  If (CLng(Session("uid")) = 0) Then
	...
	If Len(Request.Form("txtStdAccountId")) <> 0 Then
		Call DeleteStdAccount(CLng(Request.Form("txtStdAccountId")), _
													CStr(Request.Form("cboParentNewRole")))
	End If
	
  Sub DeleteStdAccount(nStdAccountId, sParentNewRole)
		Dim oStdAccountMgr, sLogFile
		'***************************
		On Error Resume Next
		Set oStdAccountMgr = Server.CreateObject("EFAStdActBS.CStdAccount")
		sLogFile = oStdAccountMgr.Delete(Session("sAppServer"), CLng(nStdAccountId), _
																		 CLng(Session("uid")), CStr(sParentNewRole))
