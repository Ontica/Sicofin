<%
  ...
  If (CLng(Session("uid")) = 0) Then
	...
	If Len(Request.QueryString("id")) <> 0 Then
		Call SaveSectors(CLng(Request.QueryString("id")))
  ...
  Sub SaveSectors(nStdAccountId)
		Dim aSectors(), oStdAccount, sSectorsList, sControlSectorsList
		'*************************************************************
		Set oStdAccount = Server.CreateObject("EFAStdActBS.CStdAccount")
		sSectorsList				= Request.Form("txtSectors")
		sControlSectorsList = Request.Form("txtSectorRoles")
		oStdAccount.AddSectors Session("sAppServer"), CLng(nStdAccountId), CStr(sSectorsList), _
													 CStr(sControlSectorsList), CDate(Request.Form("txtFromDate"))
