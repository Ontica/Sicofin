<%
  ...
  Else
	...
	Sub AddItems()
		...
		'*************************
		gsTitle  = "Asistente para agregar cuentas: sectores"
		gbEdit   = False
		gnItemId = 0
	...
	Sub EditItems(nItemId)
		Dim oStdAccount
		'*******************
		gsTitle  = "Agregar sectores a la cuenta"
		gbEdit = True
...
<% Else %>
  nCount = countCheckBoxes('chkSectors', false);
	if (nCount == 0) {
	  alert("Requiero la selección de al menos un nuevo sector.");
	  return false;
  }
  document.frmEditor.txtSectors.value = setFormArray('chkSectors', false);
<% End If %>
<% If (gnItemId = 0) Then %>
	document.frmEditor.action = 'add_sectors_roles.asp';
<% Else %>
	document.frmEditor.action = 'add_sectors_roles.asp?id=<%=gnItemId%>';
<% End If %>
document.frmEditor.submit();
return true;
}
