<%
  Option Explicit
	...
	gsReturnPage = "../sector_editor.asp"
  ...
  Sub SaveItem(nItemId)
		Dim oSector, oRecordset
		'**********************
		On Error Resume Next
		Set oSector = Server.CreateObject("AOGralLedger.CSector")
		Set oRecordset = Server.CreateObject("ADODB.Recordset")	
		Set oRecordset = oSector.GetSectorRS(Session("sAppServer"), CLng(nItemId))	
		oRecordset("nombre_sector") = Request.Form("txtName")
		oRecordset("clave_sector")	= Request.Form("txtClave")
		oRecordset("descripcion")		= Request.Form("txtDescription")
		oSector.SaveSector Session("sAppServer"), (oRecordset), CLng(nItemId)		
		oRecordset.Close
...
<tr>
	<td bgColor="khaki"><b>La información del sector fue enviada satisfactoriamente.</b></td>
</tr>
<tr><td><br><b>¿Qué desea hacer?</b></td></tr>
<tr>
	<% If CLng(Request.Form("txtItemId")) = 0 Then %>
	<td>
		<a href='<%=gsReturnPage%>'>Agregar otro sector</a>
	</td>	
	<% Else %>
	<td>
